                        ...
                        <div class="modal-body">
                            <form class="form-horizontal" id="uForm">
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="file" class="form-control" id="pic" placeholder="头像"
                                           name="pic">
                                </div>
                            </form>
                        </div>
...
<script>
    ...
    function getList(pageNum) {
        ...
        $.ajax({
            url: 'http://localhost:4444/user/allPage.action',
            type: 'GET',
            dataType: 'json',
            data: {
                "pageSize": pageSize,
            ...
            success: function (data) {
                ...
                console.log(data);
                if ((data.code == 8007) && (data.status == 200)) {
                    ...
                    for (var i = 0; i < list.length; i++) {
                        ...
                        (
                            ...
                            "<tr id='tr" + user.id + "'><td>" + user.id + "</td><td>"
                            + user.userName + "</td><td>"
                            + user.userPwd + "</td><td>"
                            + user.sex + "</td><td>"
                            + user.age + "</td><td>"
                            ...
                            + user.pic + "</td><td>"
                            + user.state + "</td><td>" +
                            "<a href='${basePath}/admin/user/userEdit.jsp?id=" + user.id + "' class='btn btn-warning' id='form-update'>编辑</a> </td><td>" +
                            "<a href='javascript:void(0);' class='btn btn-danger' onclick='confirmDelete(" + user.id + ")'>删除</a></td></tr>"
                        );
                    }
    ...
    $("#search-button").click(function () {
        ...
        if (content === "") {
            ...
            $.ajax({
                url: "http://localhost:4444/user/findById.json.action", // 假设是根据用户名搜索，你可以根据实际需要修改这个URL
                data: {id: content},
                method: "GET",
                success: function (data) {
                    console.log(data);
                    if ((data.code == 8001) && (data.status == 200)) {
        ...
        } else if (condition === "username" && content !== "") {
            // 如果搜索条件有效，根据条件和内容搜索并显示用户信息
            $.ajax({
                url: "http://localhost:4444/user/findByName.json.action",
                data: {name: content},
                method: "GET",
                success: function (data) {
                    console.log(data);
                    if ((data.code == 8002) && (data.status == 200)) {
                        $("#myContent tbody").empty(); // 清空之前的内容
...
<script>
    ...
    function deleteUser(id) {
        ...
        $.ajax({
            url: "http://localhost:4444/user/delete.json.action",
            data: {id: id},
            method: "POST",
            success: function (data) {
...
<script>
    ...
    function insert() {
        if (check()) {
            ...
            console.log("insert");
            $.ajax({
                url: 'http://localhost:4444/user/insert.json.action',
                type: 'POST',
                dataType: 'JSON',
                data: {
                    'userName': userName,
                ...
                success: function (data) {
                    ...
                    if (data.code == 8003) {    //比较业务代码是否正确
                        alert(data.msg);    //在页面提示信息
                        if (data.status == 200) {
                            clearForm();    //成功后清除表单
                            window.location.href = "${basePath}/admin/user/user.jsp";
                        }
                    } else {
    ...
    function update() {
        if (check()) {
            ...
            $.ajax({
                url: 'http://localhost:4444/user/update.json.action',
                type: 'POST',
                dataType: 'JSON',
    ...
    /*
        document.getElementById("form-update").addEventListener("click", function (event) {
            event.preventDefault();
            console.log("update => findById prevnet");
            // 发送GET请求
            $.ajax({
                url: $(this).attr('href'),
                type: 'GET',
                success: function (result) {
                    // 将返回的数据解析为JSON格式(JSON对象可以直接使用，这里是JSON对象的字符串形式)
                    var data = JSON.parse(result);

                    // 填充到指定的input/select元素中
                    $('#user-Create-Edit #uForm input[name="id"]').val(data.id);
                    $('#user-Create-Edit #uForm input[name="userName"]').val(data.userName);
                    $('#user-Create-Edit #uForm input[name="userPwd"]').val(data.userPwd);
                    $('#user-Create-Edit #uForm select[name="sex"]').val(data.sex);
                    $('#user-Create-Edit #uForm input[name="telephone"]').val(data.telephone);
                    $('#user-Create-Edit #uForm input[name="email"]').val(data.email);
                    $('#user-Create-Edit #uForm input[name="address"]').val(data.address);
                    $('#user-Create-Edit #uForm select[name="state"]').val(data.state);
                    $('#user-Create-Edit #uForm input[name="age"]').val(data.age);
                    $('#user-Create-Edit #uForm input[name="pic"]').val(data.pic);
                },
                error: function () {
                    alert('请求失败');
                }
...
<script>
    function ajaxSubmit() {
        ...
        $.ajax({
            url: 'http://localhost:4444/user/upload.action',
            data: fileForm,
            type: 'post',
            dataType: 'json',
            cache: false,
